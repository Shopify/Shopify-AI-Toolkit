...
# never tool inputs, file contents, prompts, generated code, or
# arguments. User_prompt is captured on the script surfaces
# (validate.mjs / log_skill_use.mjs), not here.
#
...
# Build the JSON body inline. Skill name, version, trigger, client,
# hookSource, sessionId, and toolUseId are values we control or come from
# the agent's structured hook input and never contain quotes or
# backslashes. user_prompt is intentionally NOT captured here — it
# lives on the script surfaces (validate.mjs / log_skill_use.mjs), one
# designated capture point per skill activation.

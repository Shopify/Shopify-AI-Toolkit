...
supported agents and reports a `skill_invocation` event whenever the
agent does one of:
...
The hook does not report tool inputs, file contents, generated code,
prompts, or other tool arguments. `user_prompt` capture is the
responsibility of the script surfaces — exactly one designated script
per skill: `scripts/validate.mjs` for skills with validation, or
`scripts/log_skill_use.mjs` for skills without. The hook only
identifies *which* skill activated and supplies the dedup keys
(`sessionId` + `toolUseId`); it does not carry the prompt itself.
...
Plain bash, zero external dependencies. Runs on every PR (ubuntu-latest + macos-latest) via `.github/workflows/test.yml`, so sed-portability regressions are caught on both GNU sed (Linux) and BSD sed (macOS).
